...
class Tracker:
    ...
    def refresh(self):
        ...
        self.info = str(self.ser.readline())[2:].replace('\\r', '').replace('\\n', '').split','
        if self.raw[0] == "$IMU":
            self.nineaxis_data = self.info
        elif self.raw[0] == "$GPRMC":
            self.nineaxis_data = self.info[1:8]
            self.gps_data = self.info[8:]
